...
const manageSpamRequests= {
      ...
      getPotentialSpamUsers:async({currentPage, pageSize})=>{
        const endpoint = `${process.env.STRAPI_ADMIN_BACKEND_URL}/strapi-manage-spam/get-potential-spammers?currentPage=${currentPage}&pageSize=${pageSize}`
        return await request(endpoint, {method:'GET'})
     },
      deleteAllPotentialSpamUsers:async({currentPage, pageSize})=>{
        const endpoint = `${process.env.STRAPI_ADMIN_BACKEND_URL}/strapi-manage-spam/delete-potential-spammers?currentPage=${currentPage}&pageSize=${pageSize}`
        return await request(endpoint, {method:'GET'})
     },
